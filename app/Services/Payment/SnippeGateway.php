...
class SnippeGateway implements HotspotPaymentGateway
{
    ...
    public function initiatePayment(Transaction $transaction, array $payload = []): array
    {
        $response = Http::timeout(20)
            ->acceptJson()
            ->withToken((string) config('payment.snippe.api_key', ''))
            ->post(
                $this->url((string) config('payment.snippe.initiate_path', '/api/v1/charges/mobile-money')),
                [
                    'amount' => (float) $transaction->amount,
                    'msisdn' => $payload['phone_number'] ?? $transaction->phone_number,
        ...
        return [
            ...
            'provider_reference' => $data['id'] ?? $data['transaction_id'] ?? $data['provider_reference'] ?? null,
            'gateway_fee' => (float) ($data['fee'] ?? 0),
            'message' => $data['message'] ?? ($success ? 'Snippe accepted payment request.' : 'Snippe rejected payment request.'),
            'raw' => $data,
        ];
    }
    ...
    public function checkPaymentStatus(string $providerReference): array
    {
        $response = Http::timeout(20)
            ->acceptJson()
            ->withToken((string) config('payment.snippe.api_key', ''))
            ->get($this->url((string) config('payment.snippe.status_path', '/api/v1/charges')) . '/' . $providerReference);
        ...
        return [
            ...
            'status' => $status,
            'provider_reference' => $data['id'] ?? $data['transaction_id'] ?? $providerReference,
            'gateway_fee' => (float) ($data['fee'] ?? 0),
    ...
    {
        $secret = (string) config('payment.snippe.webhook_secret', '');
        ...
        $signature = $this->headerValue($headers, 'x-webhook-signature')
            ?? $this->headerValue($headers, 'x-snippe-signature');
    ...
    public function normalizeWebhookPayload(array $payload): array
    {
        ...
        return [
            ...
            'provider_reference' => $payload['id'] ?? $payload['transaction_id'] ?? $payload['provider_reference'] ?? null,
    ...
    {
        $baseUrl = rtrim((string) config('payment.snippe.base_url', ''), '/');
    ...
    {
        $callbackPath = (string) config('payment.snippe.webhook_url', '/api/v1/webhooks/snippe');
